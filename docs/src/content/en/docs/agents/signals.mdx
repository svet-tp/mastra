...
| `low` | Batch with `summaryAt` and later deliver one notification summary | Persist in the inbox only |

Low-priority idle notifications are intentionally silent. Give the agent the `notification-inbox` tool when it should list or read those records.
